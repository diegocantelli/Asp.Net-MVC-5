...
namespace DemoCrud.Controllers
{
    public class LivrosController : Controller
    {
        ...
        {
            string ordenacao = Request["sort[Titulo]"];
            ...
            var livrosPaginados = livros.OrderBy(l => l.Titulo).Skip((current-1) * rowCount).Take(rowCount);
            return Json(new { rows = livrosPaginados.ToList(),
                              current = current,
                              rowCount = rowCount,
                              total=total}, 
